  ...
  const fetchConfirmedBookings = async () => {
    ...
    try {
      ...
      if(response.status === 200){
        const confirmed = data.filter((d,i) => d.status === 'done');
      ...
      contentContainerStyle={{
       ...
       data={bookingData}
        ...
        const formattedDate = formatted.split(',')[2];
          return <TaskCards navigation={navigation} status={item.status} complete={true} image={item.user.profile_image} id={item.id} name={item.user.fullname} time={`${formattedTime}, ${formattedYear} | ${formattedDate}`}/>;
      }}
