...
const BookingPending = ({navigation}) => {
  ...
  const fetchAcceptedBooking = async () => {
    ...
    const filterAcceptBooking = filterRequestBooking.filter((i) => (i.status === 'confirmed' || i.status === 'pending'));
    console.log(filterAcceptBooking)
    setMyBooking(filterAcceptBooking)
    const user_id = filterAcceptBooking[0].service.user
    console.log(filterAcceptBooking[0].service)
      ...
      contentContainerStyle={{
       ...
       data={myBooking}
        ...
        const formattedDate = formatted.split(',')[2];
          return <TaskCards DropDown={true} navigation={navigation} statuses={item.status} image={img ? img : null} pending={true} id={item.id} name={item.service.title} time={`${formattedTime}, ${formattedYear} | ${formattedDate} `}/>;
      }}
